...
ui <- fluidPage(
  # tags$head(includeCSS("bootstrap.css"), ),# use a certain .css style sheet
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Input an action button ----
      actionButton(inputId = "button", label = "GO!"),
      
      # Input: file input ----
      fileInput(inputId = "file", label = "Upload a file (RDS)", multiple = FALSE, accept = c(".rds")),
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel(title = "Info",
                           h3("App description"),
                           "This is the course", strong("shiny app"), 
                           "This is the course shiny app. It is created during the course exercises using the europe.rds data. 
...
server <- function(input, output, session) {
  ...
  output$download <- downloadHandler(
    ...
    content = function(file) {
      write.csv(year_df(), file, row.names = FALSE)
    }
